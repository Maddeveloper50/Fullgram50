 ...
 */
package ir.supergram.messenger.exoplayer.extractor;

import ir.supergram.messenger.exoplayer.C;
import ir.supergram.messenger.exoplayer.MediaFormat;
import ir.supergram.messenger.exoplayer.MediaFormatHolder;
import ir.supergram.messenger.exoplayer.ParserException;
import ir.supergram.messenger.exoplayer.SampleHolder;
import ir.supergram.messenger.exoplayer.SampleSource;
import ir.supergram.messenger.exoplayer.SampleSource.SampleSourceReader;
import ir.supergram.messenger.exoplayer.TrackRenderer;
import ir.supergram.messenger.exoplayer.drm.DrmInitData;
import ir.supergram.messenger.exoplayer.upstream.Allocator;
import ir.supergram.messenger.exoplayer.upstream.DataSource;
import ir.supergram.messenger.exoplayer.upstream.DataSpec;
import ir.supergram.messenger.exoplayer.upstream.Loader;
import ir.supergram.messenger.exoplayer.upstream.Loader.Loadable;
import ir.supergram.messenger.exoplayer.util.Assertions;
import ir.supergram.messenger.exoplayer.util.Util;
 ...
 *
 * <ul>
 * <li>MP4, including M4A ({@link ir.supergram.messenger.exoplayer.extractor.mp4.Mp4Extractor})</li>
 * <li>fMP4 ({@link ir.supergram.messenger.exoplayer.extractor.mp4.FragmentedMp4Extractor})</li>
 * <li>Matroska and WebM ({@link ir.supergram.messenger.exoplayer.extractor.webm.WebmExtractor})</li>
 * <li>Ogg Vorbis/FLAC ({@link ir.supergram.messenger.exoplayer.extractor.ogg.OggExtractor}</li>
 * <li>MP3 ({@link ir.supergram.messenger.exoplayer.extractor.mp3.Mp3Extractor})</li>
 * <li>AAC ({@link ir.supergram.messenger.exoplayer.extractor.ts.AdtsExtractor})</li>
 * <li>MPEG TS ({@link ir.supergram.messenger.exoplayer.extractor.ts.TsExtractor})</li>
 * <li>MPEG PS ({@link ir.supergram.messenger.exoplayer.extractor.ts.PsExtractor})</li>
 * <li>FLV ({@link ir.supergram.messenger.exoplayer.extractor.flv.FlvExtractor})</li>
 * <li>WAV ({@link ir.supergram.messenger.exoplayer.extractor.wav.WavExtractor})</li>
  ...
  static {
    ...
    try {
      DEFAULT_EXTRACTOR_CLASSES.add(
          Class.forName("ir.supergram.messenger.exoplayer.extractor.webm.WebmExtractor")
    ...
    try {
      DEFAULT_EXTRACTOR_CLASSES.add(
          Class.forName("ir.supergram.messenger.exoplayer.extractor.mp4.FragmentedMp4Extractor")
              .asSubclass(Extractor.class));
    } catch (ClassNotFoundException e) {
      // Extractor not found.
    }
    try {
      DEFAULT_EXTRACTOR_CLASSES.add(
          Class.forName("ir.supergram.messenger.exoplayer.extractor.mp4.Mp4Extractor")
              .asSubclass(Extractor.class));
    } catch (ClassNotFoundException e) {
      // Extractor not found.
    }
    try {
      DEFAULT_EXTRACTOR_CLASSES.add(
          Class.forName("ir.supergram.messenger.exoplayer.extractor.mp3.Mp3Extractor")
              .asSubclass(Extractor.class));
    } catch (ClassNotFoundException e) {
      // Extractor not found.
    }
    try {
      DEFAULT_EXTRACTOR_CLASSES.add(
          Class.forName("ir.supergram.messenger.exoplayer.extractor.ts.AdtsExtractor")
              .asSubclass(Extractor.class));
    } catch (ClassNotFoundException e) {
      // Extractor not found.
    }
    try {
      DEFAULT_EXTRACTOR_CLASSES.add(
          Class.forName("ir.supergram.messenger.exoplayer.extractor.ts.TsExtractor")
              .asSubclass(Extractor.class));
    } catch (ClassNotFoundException e) {
      // Extractor not found.
    }
    try {
      DEFAULT_EXTRACTOR_CLASSES.add(
          Class.forName("ir.supergram.messenger.exoplayer.extractor.flv.FlvExtractor")
              .asSubclass(Extractor.class));
    } catch (ClassNotFoundException e) {
      // Extractor not found.
    }
    try {
      DEFAULT_EXTRACTOR_CLASSES.add(
          Class.forName("ir.supergram.messenger.exoplayer.extractor.ogg.OggExtractor")
              .asSubclass(Extractor.class));
    } catch (ClassNotFoundException e) {
      // Extractor not found.
    }
    try {
      DEFAULT_EXTRACTOR_CLASSES.add(
          Class.forName("ir.supergram.messenger.exoplayer.extractor.ts.PsExtractor")
              .asSubclass(Extractor.class));
    } catch (ClassNotFoundException e) {
      // Extractor not found.
    }
    try {
      DEFAULT_EXTRACTOR_CLASSES.add(
          Class.forName("ir.supergram.messenger.exoplayer.extractor.wav.WavExtractor")
              .asSubclass(Extractor.class));
    } catch (ClassNotFoundException e) {
      // Extractor not found.
    }
    try {
      DEFAULT_EXTRACTOR_CLASSES.add(
          Class.forName("ir.supergram.messenger.exoplayer.ext.flac.FlacExtractor")
              .asSubclass(Extractor.class));
    } catch (ClassNotFoundException e) {
      // Extractor not found.
    }
